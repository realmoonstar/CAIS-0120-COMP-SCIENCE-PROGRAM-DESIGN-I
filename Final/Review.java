...
//Write a Student class that extends the Person class by adding an id property and a number of courses taken property, also change the functionality of the growOlder behavior to increment the age by twice the amount of years if the student is taking more than ten courses at a time.
//Do this in two different ways, first do it by using Containment/Composition and then do it by using Inheritance.
//4 – The constructor of a Balloon class takes as parameter a string specifying the color of the balloon. Write some code to declare and allocate an array of ten balloons and initialize all the balloons to be white.
